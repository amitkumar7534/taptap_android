...
import com.example.myapplication.adapter.TopBusinessAdapter
...
import com.yookoo.discoveraddis.databinding.FragmentBusinessBinding
...
class BusinessFragment : BaseFragment<FragmentBusinessBinding>(),ViewClickCallback {
    ...
    lateinit var postAdapter : TopBusinessAdapter
...
//        }
        ...
        getBaseActivity().baseViewModel.repository.getAllBusiness(apiListener(),category).observe(requireActivity()) {
            if(it.success){
                if(it.data.isNullOrEmpty()){
                    ...
                    adapter = BusinessesAdapter(requireActivity(),false,distaceList,this)
                    ...
                    for (i in it.data.indices) {
                        distance(
                            origin.latitude,
                            origin.longitude,
                            it.data[i].latlng.split(",").get(0).toDouble(),
                            it.data[i].latlng.split(",").get(1).toDouble()
                        )
                    }

                    Log.e("cdsc ds", distaceList.toString())
                    if (activity != null) {
                        adapter = requireActivity()?.intent?.getBooleanExtra("distance", false)
                            ?.let { it1 ->
                                BusinessesAdapter(requireActivity(), it1, distaceList, this)
                            }
                        adapter?.list!!.clear()
    ...
    fun getTopBusinesses(category:String){
        getBaseActivity().baseViewModel.repository.getTopBusiness(apiListener(),category).observe(requireActivity()) {
            if(it.success){
                if(!it.data.isNullOrEmpty()){
                    if (activity!=null) {
                        ...
                        tBinding!!.businessTopView.setLayoutManager(HorizontalLayout);
                        postAdapter = TopBusinessAdapter(requireContext())
                        postAdapter.list = it.data as ArrayList<Data>
                        tBinding!!.businessTopView.adapter = postAdapter
                    }
                }
            }
        }
