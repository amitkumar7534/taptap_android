...
import com.example.myapplication.fragments.home.BusinessFragment
import com.example.myapplication.fragments.home.PostFragment
import com.example.myapplication.ui.base.BaseFragment
import com.example.myapplication.ui.callback.ViewClickCallback
import com.example.myapplication.utils.CommonMethods.replaceFragmentActivityWithoutStack
...
import com.yookoo.discoveraddis.databinding.FragmentAllBusinessesBinding
...
//        tBinding.userName.setText("Hi "+ appSession().getString(Constant.USER_NAME).toString())
        ...
        tBinding.messageView.setOnClickListener {
            val i = Intent(requireActivity(), CommonActivity::class.java)
            i.putExtra(getString(R.string.fragmentName), getString(R.string.chat_fragment))
            startActivity(i)
        }
        ...
        val slideAnimation = AnimationUtils.loadAnimation(requireActivity(), R.anim.bell_icon)
        tBinding.bell.startAnimation(slideAnimation)

        tBinding.businessView.setOnClickListener {
            replaceFragmentActivityWithoutStack(
                requireActivity().supportFragmentManager,
                BusinessFragment(),
                R.id.splashContainer)

            tBinding.businessView.setBackground(ContextCompat.getDrawable(requireActivity(), R.drawable.bg_search))
            tBinding.postView.setBackground(null)

            tBinding.postView.setTextColor(Color.parseColor("#ffffff"));
            tBinding.businessView.setTextColor(Color.parseColor("#000000"));

        }


        tBinding.postView.setOnClickListener {
            ...
            tBinding.businessView.setTextColor(Color.parseColor("#ffffff"));
            tBinding.postView.setTextColor(Color.parseColor("#000000"));

            tBinding.postView.setBackground(ContextCompat.getDrawable(requireActivity(), R.drawable.bg_search))
            tBinding.businessView.setBackground(null)
...
//        tBinding.notificationView?.visibility=View.VISIBLE
    ...
    fun getNotification() {
        getBaseActivity().baseViewModel.repository.getNotification(apiListener(), appSession().getString(Constant.USER_ID).toString()).observe(this) {
            hideProgress()
            if (it.success == true) {
